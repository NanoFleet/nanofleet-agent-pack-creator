...
## HEARTBEAT.md
Free-form markdown describing what the agent should do each time the heartbeat triggers.
...
Omit schedule or cron expressions — timing is controlled externally via the
HEARTBEAT_INTERVAL environment variable.

</file_specifications>

<skill_calibration>
Skill descriptions in the YAML frontmatter control when a skill activates.
Calibrate them carefully:
- Too broad ("Help with security") causes false positives — the skill triggers when
  it should not.
- Too narrow ("Write CVE report for PHP SQL injection via nuclei") means the skill
  almost never triggers.
- Well-calibrated descriptions state the WHEN and WHAT in 1-2 sentences without
  over-specifying the HOW.

<example title="well-calibrated description">
description: Analyze web application vulnerabilities and produce structured security
...
description: Run nuclei scanner against PHP apps to find SQL injection CVEs and format
as CVSS 3.1 report
</example>
</skill_calibration>
...
- capability-uplift: Teaches the agent something it could not do reliably without this
  skill — specific techniques, proprietary formats, complex workflows.
...
- encoded-preference: Sequences actions the agent could do individually, but in a specific
  order, style, or convention relevant to this user's context.
...
5. Generate HEARTBEAT.md only if the research indicates periodic tasks; otherwise output
   an empty string.
6. Set agentName to a clean kebab-case slug (e.g., "web-security-researcher").
</instructions>

<output_format>
Return structured JSON matching the schema with all pack files populated. Use domain-specific
vocabulary and reference real tools from the research throughout.
</output_format>
```
